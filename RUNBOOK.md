...
## Captured (2026-05-28) — Sprint 35 Tabular Review (Stage A engine + Stage B UI)

- **Recipe discoverability now wired (closes the Sprint-34 carry-forward)**: `ensureBundledRecipes()` in `ui/desktop/src/main.ts` copies `oscar/recipes/*.yaml` → the recipes dir Summon scans at app launch — `~/.config/goose/recipes/` on Linux, or `<GOOSE_PATH_ROOT>/config/recipes` when that env is set (matches `crates/goose/src/config/paths.rs` `config_dir()`). Idempotent (content-compare), never touches sibling recipes. A launched app now leaves `~/.config/goose/recipes/tabular-cell-extractor.yaml` in place — no manual copy needed.
- **Bundle staging wired**: `prepare-oscar-bundle.js` now stages the tabular MCP (`SIBLING_MCPS.tabular` → esbuild → `RESOURCES/mcps/tabular/index.js`; dir name is `tabular`, not `oscar-tabular`, to match `resolveTabularBundle`) and `oscar/recipes` (`prepareRecipes()` → `RESOURCES/recipes/`). The bundled-MCP spawn-smoke gained a `tabular` check with `env:{OSCAR_MATTER_DIR:sandbox}` — passes (`tabular: pass`).
- **Packaged binary rebuilt**: `cd ui/desktop && node scripts/prepare-oscar-bundle.js && pnpm run package` → `out/Oscar-GC-linux-x64/oscar-gc` (now carries the tabular MCP, the staged recipe, the recipe-copy code, and the Stage B UI). Python/Node staging is cached (`.oscar-bundle-cache`) so re-runs skip downloads. (Full `.deb` via `bundle:oscar-linux` not run this sprint.)
- **DEV note**: in a dev build (`oscarResourcesRoot` null) the recipe-copy source falls back to `/srv/projects/goose/oscar/recipes` and the tabular MCP loads from `oscar/mcps/tabular/dist/index.js` (run `pnpm -C oscar/mcps/tabular run build` first).
- **Deterministic engine/manifest demo (no LLM)**: `node oscar/mcps/tabular/demo-manifest.mjs "<matter-working-dir>" [fixtures-dir]` drives the real MCP over the 5 markdown fixtures → a real grounded manifest (`outputs/tabular-review/<id>/manifest.json`). Used to feed the Stage B UI for screenshots; fixtures + a 50-doc generator live at `docs/dogfood/sprint-35/`.
- **Stage B screenshot recipe**: `bash scripts/dogfood/dogfood.sh boot <s>` → `dogfood.sh eval 'window.electron.matters.create("commercial", {slug,name,kind:"contract-review",subject:{type:"contract",label},counterparty:null,stakeholder:null,privileged:false,key_facts:""})'` (subject.type MUST be a valid enum: contract/person/entity/transaction/…) → `node oscar/mcps/tabular/demo-manifest.mjs "<working_dir>"` → `DOGFOOD_SCREENSHOT_BASE=docs/screenshots/sprint-35 dogfood.sh goto '/tabular-review?areaId=commercial&slug=<slug>&reviewId=<id>'`. A demo Commercial matter "Project Atlas" exists at `~/Documents/Oscar GC/Commercial/Project Atlas` on lq-vps.
- **pnpm dependency blocker (open)**: `pnpm add` in `ui/desktop` fails with `ERR_PNPM_EXOTIC_SUBDEP` — pnpm 11's `blockExoticSubdeps` rejects electron-forge's transitive git dep `@electron/node-gyp` on a fresh full-resolve. So `@tanstack/react-table` + `@dnd-kit/*` (for grid dnd/filter) and `pdfjs-dist`/`docx-preview` (for the PDF/DOCX citation tier) were **not** installed; Stage B ships a plain-React grid + text-tier highlight instead. To add deps later, decide the `blockExoticSubdeps` policy in `ui/pnpm-workspace.yaml` deliberately (it affects `.deb` build reproducibility) before re-resolving.
