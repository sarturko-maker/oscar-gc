#!/usr/bin/env bash
# Sprint M4 Playbooks subsystem visual verification wrapper. Clones
# capture-m3.sh; invokes ui/desktop/scripts/capture-m4.js — drives 5
# states (empty pane, two files listed with mixed scope, always-on chip
# toggled, Layer 1 renderBlock injection diagnostic, 50K-char budget
# rejection warning).
#
# Usage:
#   bash scripts/capture-m4.sh --out-dir docs/screenshots/sprint-m4
#
# Env knobs (same as capture-m3.sh):
...
if [[ -f "$API_KEY_ENV" ]]; then
  ...
  echo "[capture-m4] WARN: $API_KEY_ENV not found; MINIMAX_API_KEY will be unset" >&2
fi
...
echo "[capture-m4] starting Xvfb :$DISPLAY_NUM ($SCREEN_GEOMETRY)"
...
if [[ ! -e "/tmp/.X11-unix/X${DISPLAY_NUM}" ]]; then
  echo "[capture-m4] Xvfb failed to come up; tail /tmp/xvfb-${DISPLAY_NUM}.log:" >&2
...
cd "$REPO_ROOT"
node "ui/desktop/scripts/capture-m4.js" "$@"
RC=$?
exit "$RC"
