#!/usr/bin/env bash
# Sprint M7 Forge Mode D (modify area) visual verification wrapper.
# Clones capture-m6.sh; invokes ui/desktop/scripts/capture-m7.js — drives
# 6 deterministic states (Edit-link visible / Forge Mode-D opens /
# Mode-D recipe diagnostic / description_override round-trip /
# enabled_mcps filter round-trip / validator rejection auto-reverts).
#
# Live agent turns deferred to M8 Crostini dogfood. The visual harness
# validates the wiring; the recipe-injection diagnostic mirrors M4/M5/M6's
# recipe-content verification.
#
# Usage:
#   bash scripts/capture-m7.sh --out-dir docs/screenshots/sprint-m7
#
# Env knobs (same as capture-m6.sh):
...
if [[ -f "$API_KEY_ENV" ]]; then
  ...
  echo "[capture-m7] WARN: $API_KEY_ENV not found; MINIMAX_API_KEY will be unset" >&2
fi
...
echo "[capture-m7] starting Xvfb :$DISPLAY_NUM ($SCREEN_GEOMETRY)"
...
if [[ ! -e "/tmp/.X11-unix/X${DISPLAY_NUM}" ]]; then
  echo "[capture-m7] Xvfb failed to come up; tail /tmp/xvfb-${DISPLAY_NUM}.log:" >&2
...
cd "$REPO_ROOT"
node "ui/desktop/scripts/capture-m7.js" "$@"
RC=$?
exit "$RC"
