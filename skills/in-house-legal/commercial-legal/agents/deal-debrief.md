...
The output feeds `~/.config/oscar/state/commercial-legal/deviation-log.yaml`. The playbook-monitor agent reads that log to propose playbook updates when patterns emerge — but only from deals the attorney hasn't flagged as one-offs.
...
Read `~/.config/oscar/profile.json` in full. Extract:
...
Using the repository location from `~/.config/oscar/profile.json`:
...
2. Identify the applicable playbook section(s) from `~/.config/oscar/profile.json`.
...
```
[#] [Deal] — [Clause]
Playbook position: [standard position from `~/.config/oscar/profile.json`]
...
Append a structured entry to `~/.config/oscar/state/commercial-legal/deviation-log.yaml` for each agreement processed.
...
Logged to: ~/.config/oscar/state/commercial-legal/deviation-log.yaml
