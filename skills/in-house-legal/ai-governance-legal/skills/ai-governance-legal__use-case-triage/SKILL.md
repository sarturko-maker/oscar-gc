---
name: ai-governance-legal__use-case-triage
description: >
...
1. Read `~/.config/oscar/profile.json`. Confirm registry is populated — if not, stop and direct to setup.
...
```
ai-governance-legal__use-case-triage "Sales team wants to score leads with AI automatically"
```
...
## Read `~/.config/oscar/profile.json` first

Before triaging, always read `~/.config/oscar/profile.json`. The use case registry and red lines there
...
>
> **Two choices:**
> - Run Oscar GC onboarding (2 minutes) to configure your profile, then I'll triage tailored to YOUR practice.
...
> "That was a generic run against default assumptions. Run Oscar GC onboarding to get output calibrated to YOUR practice — your registry, your jurisdiction, your risk appetite. 2 minutes."

**Jurisdictional scope.** Triage applies the registry, red lines, and governance tiers configured for the regulatory footprint in `~/.config/oscar/profile.json`. AI rules vary materially by jurisdiction — an APPROVED classification in one footprint may be CONDITIONAL or prohibited in another. If deployment touches a jurisdiction not in the footprint, surface that and re-triage rather than extending by analogy.
...
Check the use case registry in `~/.config/oscar/profile.json` for a direct or close match.
...
The APPROVED / CONDITIONAL / NOT APPROVED buckets, the red-line definitions, and the CONDITIONAL required-controls list all come from `~/.config/oscar/profile.json` → `## AI use case triage criteria` and `## Use case registry`. If the playbook doesn't define a criterion the use case turns on, ask the user: "Your playbook doesn't cover [specific question]. What's your default position? I'll add it to `~/.config/oscar/profile.json` so the next triage is consistent."

**Before issuing an APPROVED classification (approving an AI use case for deployment):** Read `## Who's using this` in `~/.config/oscar/profile.json`. If the Role is Non-lawyer:
...
**Governance tier:** [Standard / Elevated / High — per `~/.config/oscar/profile.json`]
...
> an AI impact assessment. Use `pia-generation [use case]`, if the
...
> Use `launch-review`, if the plugin is installed — it will detect
...
> "Add to `~/.config/oscar/profile.json` → Use case registry. This means next time the same request
