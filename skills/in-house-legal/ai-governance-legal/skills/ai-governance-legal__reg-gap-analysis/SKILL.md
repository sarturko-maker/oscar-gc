---
name: ai-governance-legal__reg-gap-analysis
description: >
...
1. Read `~/.config/oscar/profile.json`. Confirm regulatory footprint and use case registry are populated.
...
4. Extract requirements. Diff against current state in `~/.config/oscar/profile.json`.
...
```
ai-governance-legal__reg-gap-analysis "EU AI Act high-risk provisions"
```
...
(per `~/.config/oscar/profile.json` — use case registry, vendor positions, impact assessment practices,
...
**We currently:** [what `~/.config/oscar/profile.json` / AI policy / use case registry / assessment
practice shows]
