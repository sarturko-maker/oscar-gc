...
- `matter-workspace close <slug>` — archive a matter (move to `~/.config/oscar/state/ai-governance-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.config/oscar/state/ai-governance-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.config/oscar/state/ai-governance-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.config/oscar/state/ai-governance-legal/matters/<slug>/` to `~/.config/oscar/state/ai-governance-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.config/oscar/state/ai-governance-legal/
