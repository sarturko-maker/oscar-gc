---
name: privacy-legal__reg-gap-analysis
description: >
...
1. Load `~/.config/oscar/profile.json` → privacy policy commitments, regulatory footprint, DSAR systems.
...
```
privacy-legal__reg-gap-analysis "Colorado Privacy Act"
```

```
privacy-legal__reg-gap-analysis
...
This skill diffs the new requirement against what you currently do (per `~/.config/oscar/profile.json` → Privacy policy commitments + the practices documented in PIAs) and produces a gap list with a remediation plan.
