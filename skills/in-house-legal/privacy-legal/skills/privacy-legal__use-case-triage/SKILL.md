---
name: privacy-legal__use-case-triage
description: >
...
1. Read `~/.config/oscar/profile.json`. Confirm privacy practice is configured — if not, stop and direct to setup.
...
```
privacy-legal__use-case-triage "New feature that uses behavioral data to personalize content recommendations"
```
...
Before triaging, always read `~/.config/oscar/profile.json`. The PIA trigger criteria, regulatory
...
> "That was a generic run against default assumptions. Run Oscar GC onboarding to get output calibrated to YOUR practice — your regulatory footprint, your privacy policy commitments, your risk appetite. 2 minutes."
...
Read `~/.config/oscar/profile.json` → `## PIA house style` → Trigger criteria. Apply them.
...
Read `~/.config/oscar/profile.json` → `## Privacy policy commitments`. Check the proposed activity
...
> required in addition to a PIA. Use `aia-generation [activity]`
...
> Use `launch-review` — it will detect the privacy component
