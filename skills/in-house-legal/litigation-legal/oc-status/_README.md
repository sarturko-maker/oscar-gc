...
Output from `oc-status`. Per-run folders dated by day; each contains one markdown file per matter drafted, plus a `_summary.md`.
...
Weekly (Monday AM) when scheduled. Register the schedule with `oc-status --setup-schedule`.

Ad-hoc any time with `oc-status` (default filter) or `oc-status --slug=[slug]` (one matter).
