...
- `matter-workspace close <slug>` — archive a matter (move to `~/.config/oscar/state/corporate-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.config/oscar/state/corporate-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.config/oscar/state/corporate-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.config/oscar/state/corporate-legal/matters/<slug>/` to `~/.config/oscar/state/corporate-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; `matter-workspace` explains the disabled state and suggests the Oscar GC onboarding from Settings for users who actually need matter isolation.
...
```
~/.config/oscar/state/corporate-legal/
