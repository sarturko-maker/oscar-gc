...
description: >
  ...
  per the trust level in `~/.config/oscar/profile.json`. Use when user says "send to Luminance",
...
If `~/.config/oscar/profile.json` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `~/.config/oscar/profile.json` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `~/.config/oscar/profile.json` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `~/.config/oscar/profile.json`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `~/.config/oscar/profile.json` says this — it's rare.)
...
**Trust level:** [per `~/.config/oscar/profile.json`]
...
- It doesn't replace the tool's output with its own judgment entirely — if `~/.config/oscar/profile.json` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `~/.config/oscar/profile.json`, set at cold-start based on the team's experience with the tool.
