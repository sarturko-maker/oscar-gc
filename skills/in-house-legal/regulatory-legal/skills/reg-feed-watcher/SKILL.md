...
1. Load `~/.config/oscar/profile.json` → watchlist, materiality threshold, feed config.
...
`~/.config/oscar/profile.json` → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
> **Coverage gap noticed:** Your watchlist includes [category], but only [N] feeds are configured. The source catalog lists [X] options in this category (e.g., [top 2-3 names]). Want me to suggest additions? Run the Oscar GC onboarding from Settings to update, or edit `~/.config/oscar/profile.json` directly.
...
- **Direct regulator RSS** — fetch and parse any RSS URLs in ~/.config/oscar/profile.json feed
...
Each item gets a materiality tier per `~/.config/oscar/profile.json`:
...
| ANPR (Advance Notice of Proposed Rulemaking) | Review-worthy for **strategy**, not compliance — no imposed requirements yet, but signals direction and carries a real comment deadline. Log the comment deadline. Route to `policy-diff` only as a pre-positioning analysis, not as a gap-closure diff. |
...
- **Do** log the comment deadline to `~/.config/oscar/state/regulatory-legal/comment-tracker.yaml` with `item_type: ANPR` or `item_type: RFI` so the downstream tracker can distinguish these from compliance gaps.
- **Do** include in the digest entry a line that says explicitly: "Pre-rule. Comment deadline [date]. Route to `policy-diff` only as a pre-positioning analysis (no compliance gap yet)." This primes the policy-diff skill to use its compressed pre-positioning branch rather than a full gap-closure diff.
...
- If comment tracking is enabled in ~/.config/oscar/profile.json: append to `~/.config/oscar/state/regulatory-legal/comment-tracker.yaml`
  with status "undecided" and the default comment decision owner from ~/.config/oscar/profile.json
...
1. Look for `Digest output path` in `~/.config/oscar/profile.json`. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
**Comment tracker:** [N] NPRMs with open comment decisions — run comments to review
...
This skill reads the watchlist, materiality threshold, and feed configuration from `~/.config/oscar/profile.json`. When a required value is still `[PLACEHOLDER]` or empty, say so in the output — specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run the Oscar GC onboarding from Settings or edit `~/.config/oscar/profile.json` and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with the Oscar GC onboarding from Settings or by editing `~/.config/oscar/profile.json`."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with the Oscar GC onboarding from Settings or by editing `~/.config/oscar/profile.json`."
...
- Change the materiality threshold. If the filter is wrong, edit ~/.config/oscar/profile.json.
