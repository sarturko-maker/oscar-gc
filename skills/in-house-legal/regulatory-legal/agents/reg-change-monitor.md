...
description: >
  ...
  Runs per the cadence in ~/.config/oscar/profile.json. Filters by materiality threshold so the
...
Per `~/.config/oscar/profile.json` → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `~/.config/oscar/profile.json` → watchlist, materiality threshold.
