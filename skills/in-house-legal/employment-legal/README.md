...
- **Persistent data path.** The leave register, investigation logs, and expansion trackers are written to `~/.config/oscar/state/employment-legal/`, a version-independent path that survives plugin updates. These files contain privileged and sensitive personnel information — make sure that directory is backed up and access-controlled.
...
| Skill | Does |
|---|---|
| Oscar GC onboarding | Cold-start interview — learns jurisdictional footprint + escalation rules from handbook + term memos |
| `hiring-review` | Offer letter + restrictive covenant review, jurisdiction check |
| `termination-review` | Termination review with high-risk flag detection |
| `policy-drafting [topic]` | Draft a policy with state supplements where needed |
| `wage-hour-qa [question]` | Wage/hour or general employment Q&A, jurisdiction-aware |
| `worker-classification` | Classify a proposed worker engagement and flag misclassification gaps |
| `expansion-kickoff [country]` | Kick off international expansion planning for a new country |
| `expansion-update [country]` | Update an in-progress expansion tracker |
| `investigation-open` | Open a new internal investigation matter |
| `investigation-add` | Add documents, interview notes, or observations to an open investigation |
| `investigation-query` | Ask questions against an open investigation log |
| `investigation-memo` | Draft or update the privileged investigation memo |
| `investigation-summary` | Draft an audience-specific summary from the investigation memo |
| `leave-tracker` | Check open leaves for deadline alerts and required decisions |
