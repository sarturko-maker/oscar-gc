...
Adds a new leave entry to `~/.config/oscar/state/employment-legal/leave-register.yaml` with the minimum
...
1. Read `~/.config/oscar/profile.json` → jurisdiction table and Systems section.
...
3. Using the jurisdiction table in `~/.config/oscar/profile.json`, look up the applicable leave
...
5. Write a new entry to `~/.config/oscar/state/employment-legal/leave-register.yaml` using the leave register
