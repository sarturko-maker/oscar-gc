...
### Sprint 35 — Tabular Review: Stage A engine hardening + Stage B native surface (landed 2026-05-28)

**Goal**: Make Tabular Review *real* — (A) prove the headless engine end-to-end on a live provider, and (B) build the native React full-window surface where a lawyer watches the grid fill, drills a cell to its highlighted clause, and talks to the agent — all on one screen, persistent across reopen. User decisions this session: grid = **LQ-Grid React port + lq-ai's 4-state citation model**; scope = **full Stage A + full Stage B**; Stage A provider = **MiniMax only**. Plan at `/root/.claude/plans/sprint-35-brief-md-read-the-misty-hopcroft.md`.

**Built**

- **Stage A — engine hardening + discoverability** ([[ADR-114]]): extractor recipe `response.json_schema` now **requires `quote`** (nullable; null only when answer is null) and the reviewer prompt forbids generic-header quotes; the previously-dead `isBoilerplate()` is **wired into `groundCell`** so a common-phrase quote can't substring-match to a false green (flags unless a locator corroborates). New `oscar-tabular__set_human_review` tool (verify/flag/override → folds into the cell's `human` block, recomputes `summary.verified`, single-writer per [[ADR-116]]). `tabularReviewDoctrine.ts` now insists `documents[].rel_path` is matter-relative (load-bearing for grounding) + clarifies wave draining + timed-out-task handling. **Recipe discoverability wired**: `ensureBundledRecipes()` in `main.ts` copies `oscar/recipes/*.yaml` → `~/.config/goose/recipes/` at launch (honours `GOOSE_PATH_ROOT`, verified against `crates/goose/src/config/paths.rs`); `prepare-oscar-bundle.js` stages the **tabular MCP** (`→ RESOURCES/mcps/tabular`, dir name matches `resolveTabularBundle`) + `oscar/recipes` (`→ RESOURCES/recipes`), with a spawn-smoke check (`tabular: pass`). MCP `smoke.mjs` extended (no-quote→flagged, set_human_review, persistence) — green.
- **Engine grounding proven end-to-end on real fixtures, no LLM** (`oscar/mcps/tabular/demo-manifest.mjs`): the real MCP + real grounding gate over the 5 markdown contract archetypes produced a manifest of **10 complete (all `charOverlap` grounded) / 1 flagged (the cobalt paraphrase) / 4 not_found / 0 failed / 0 pending** — proving the create→ingest(waves)→ground→finalize→persist flow and the gate's display precondition on real sources. (This is the plan's "harness-produced real manifest," decoupling Stage B from live fan-out.)
- **Stage B — native full-window surface** ([[ADR-113]], [[ADR-115]]): new **bare route `/tabular-review` outside AppLayout** so the 3-pane split owns the window; `TabularReviewView` resolves the matter via `matters.setActive` + `matters.get` and mounts the **same matter session** as a docked `BaseChat` rail in its own `ChatProvider` (single instance, no `ADD_ACTIVE_SESSION` → no duplicate SSE). **Coupling is the on-disk manifest, not shared React state** — `useManifestPoll` re-reads `manifest.json` every 2 s (gated on `updated_at`); the agent writes via MCP. Grid (`TabularGrid`/`GridCell`) ported from LQ-Grid's structure as **plain React** (sticky doc column, sortable headers, cell-click → drill); `SourceView` (offset-exact text highlight) + the citation drill (`SourceDrawer`) ported; cell-state **4-state visual vocabulary** (verified / tolerant / needs-review / unverified, human overlay wins) adopts lq-ai's model mapped onto Oscar's grounding output ([[ADR-112]]). 3 read-only IPCs (`list-reviews`/`read-manifest`/`read-source-text`, traversal+id-guarded) + preload `tabular` namespace. Right-pane **"Tabular Reviews" launcher** (registry 3-edit + always-appended in `useActiveAreaSections`) lists reviews and Opens the route — the re-entry that makes persistence real. Editorial-themed (`.oscar__tabular-*` in `main.css`). `tsc --noEmit` + eslint clean.
- **Visual verification** (`docs/screenshots/sprint-35/`): the grid renders the real Project-Atlas manifest (5 docs × 3 columns, values + status icons + summary chips, Editorial theme); clicking a verified cell opens the SourceDrawer with the green "Verified verbatim" badge and the **governing-law clause highlighted offset-exact in the source**.
- **Reuse**: confirmed the actively-maintained LQ.AI (`LegalQuants/lq-ai`, pushed 2026-05-28) is **Svelte over a cloud SaaS backend** — not portable as React components; LQ-Grid (React) is the portable code, lq-ai is the UX/citation-model reference. Reused Summon (untouched), the right-pane registry + `usePanelReader`, the `read-matter-facts` IPC pattern, `matters.*`, `BaseChat`/`ChatProvider`.

**Stage A LIVE RUN — MiniMax + real CUAD contracts (2026-05-29)** — the gating fan-out question, ANSWERED.

- Ran the engine end-to-end on **MiniMax-M2.5** over **5 real contracts from the CUAD dataset** (.txt; LOHA F-1, Centrack, Kiromic consulting, Veoneer JV, Domini trust — CUAD annotates exactly Governing Law / Cap-on-Liability / Change-of-Control). Drove a real lawyer prompt ("go through every contract … build me a tabular review with governing law, limitation of liability, change of control") through the dogfood harness against the matter session.
- **MiniMax FANNED OUT.** It listed `contracts/`, called `oscar-tabular__create_review` (5 rows, 3 columns), then `delegate`d the extractor per document, `load`ed each, `ingest`ed, and `finalize`d — manifest filled 15/15. The Sprint 32/[[ADR-107]] "delegate-shyness" (5–20% on *general discovery* tasks) did **not** block it: an explicit, task-scoped fan-out procedure (the tabular doctrine prescribes `delegate` as *the* method) overcomes the general routing reluctance. Extraction was **correct on real contracts** (PRC / Florida / Texas / Japan / Massachusetts governing law, real liability quotes). The native grid rendered the live manifest with MiniMax's portfolio analysis in the docked rail on one screen (`docs/screenshots/sprint-35/…-cuad-…png`) — also confirming `BaseChat` streams the live session in the bare route.
- **Bug found + fixed (the rel_path discipline risk, confirmed live)**: MiniMax set `create_review` `rel_path` to the **bare filename** (`x.txt`) for files living in `contracts/`, so the grounding gate (which reads `row.rel_path`) returned **`no-source` for all 30 verdicts** — cells `complete` but grey-Unverified rather than grounded-green (the extractor itself read the docs fine; only the gate's path mismatched). Fix: `matterDir.ts:readSourceText` now falls back to a **bounded basename search within the matter tree** when the literal `rel_path` doesn't resolve (model-agnostic; no doctrine reliance). Smoke extended (bare-filename rel_path → grounds). Re-grounding the real CUAD answers through the fixed MCP: **`no-source` 30 → 0; 8 cells ground via `charOverlap`**, 7 correctly flag as `no-quote` (the agent answered "None" with no citation → needs-review, not false-green).
- **Also surfaced**: `forge.config.ts` `extraResource` never listed `src/resources/recipes`, so the packaged app shipped no recipe for `ensureBundledRecipes` to copy (Summon couldn't resolve the extractor) — fixed (the test caught a packaging gap the spawn-smoke didn't); and the agent produced 6 cells/row on a re-run (duplicate column-id keys from running the flow twice) — minor, worth a dedupe-on-ingest guard. Verdict: **the engine works end-to-end on a live model with real contracts**; the no-per-model-doctrine stance holds (the fix is model-agnostic).

**Deferred / carry-forwards**

- **Live MiniMax dogfood — DONE this session** (see the Stage A LIVE RUN block above): fan-out confirmed on real CUAD contracts; rel_path-grounding bug found + fixed; `BaseChat` live-streaming in the bare route confirmed (the rail showed MiniMax's analysis). Remaining: at **portfolio scale (50 docs)** the wall-clock + wave-draining behaviour is still unmeasured (5-doc run was ~1 wave, fast); and write the **engine-verdict ADR** (the no-per-model-doctrine stance + malformed-path-is-defense-in-depth-behind-core's-`final_output`-gate) now that the result is in hand. Re-run with the **fixed bundle** to capture a grounded-green grid screenshot (this session proved green via re-ground on the same data, not via a fresh agent run on the rebuilt binary).
- **Agent returned answer "None" for absent clauses instead of null** → those cells flag as `no-quote` rather than reading `not_found`; a small doctrine/prompt nudge ("absent → answer null, not 'None'") would route them to `not_found`. And the duplicate-column-id 6-cells-per-row needs a dedupe-on-ingest guard.
- **TanStack/dnd column reorder + filter, and the PDF/DOCX original-doc render tier** (`pdfjs-dist`/`docx-preview` + `highlightText.ts` anchor highlight): **blocked by a pnpm 11 workspace policy** (`blockExoticSubdeps` rejects electron-forge's transitive git subdep on a fresh `pnpm add`). The plain-React grid + text-tier `SourceView` cover the markdown corpus fully; resolve the pnpm policy deliberately (it affects `.deb` build reproducibility), then add. The grounding gate is text-only server-side, so PDF/DOCX cells read grey "Unverified" — an MCP text-extraction-cache (`<review>/sources/<doc>.txt`) would let the gate ground PDFs too (recommended).
- **In-grid one-click verify/flag** (vs rail-typed): rail-mediated `set_human_review` works via the tool description; a programmatic rail-submit affordance is a refinement.
- **`.deb` packaging** (only `electron-forge package` run); **Phase 5** (add-column UI, rerun-as-UI, export, ensemble, RAG-baseline fidelity test). **Engine gaps**: timed-out sub-agent still leaves a row `pending` (doctrine guides retry/surface, no failed-marking path yet).

**Cross-repo**: only `sarturko-maker/oscar-gc`. New tabular MCP changes live in-repo. No Rust core touch. No new deps installed (the grid build avoided the pnpm-policy blocker).

**ADRs**: 114 (grounding-gate hardening), 115 (bare-route mount + manifest-as-coupling), 116 (human review via MCP tool).

---

### Sprint 34 — Tabular Review: Phase 1 spine + Phase 2 artifacts (Phases 1–2 landed 2026-05-28)

**Goal**: Build Tabular Review (batch document review — rows = documents, columns = extraction queries, each cell = answer + reasoning + source citation) the agentic-parallel way (one clean-context sub-agent reads each *full* document via Goose Summon; no embeddings/RAG), persisted in the matter folder and re-openable, to beat Harvey Vault / Legora Tabular Review. Motivating dogfood: a 50-contract portfolio review. Plan at `/root/.claude/plans/here-is-the-brief-prancy-allen.md`. This entry covers Phases 1–2 of a multi-sprint arc (the MCP spine + the headless engine's static artifacts); Phases 3–5 (native UI surface, right-pane launcher, differentiators) remain open.

**Built**

- **New sibling MCP `oscar/mcps/tabular/`** (TypeScript, AGPL-3.0, scaffolded from `oscar/mcps/memory`): `schema.ts` (Zod Cell/Column/Row/Manifest/RunRecord + the extractor payload contract — citation field names `source_quote`/`source_start`/`source_end`/`source_quotes` kept faithful to LQ-Grid `types/index.ts` so its SourceView/highlightText port verbatim in Phase 3); `verify.ts` (the zero-LLM grounding gate — `charOverlap`/`sectionExists`/`isBoilerplate` LIFTED with provenance from `oscar/mcps/grounding-verifier/src/verifier.ts`, not called across MCPs per PROJECT.md "no cross-MCP runtime coupling"); `merge.ts` (deterministic merge-by-(document,column), cell/row status derivation, grounding gate wired in, never-silent failed path); `store.ts` (atomic tmp→fsync→rename per-review `manifest.json` + sibling `index.json`, lifted from memory `store.ts`); `matterDir.ts` (B-class matter-dir from `OSCAR_MATTER_DIR` + traversal-guarded source-text read); `server.ts` (6 tools — create_review, ingest_results, read_manifest, add_column, rerun_cell, finalize_review — A/C-only schemas, matter dir B-class, review_id server-minted, no LLM calls, no sub-agent spawning); `index.ts` (stdio entry). `smoke.mjs` green: round-trip persistence, grounded→complete, ungrounded→flagged + confidence-low, malformed→failed (never dropped), no-source→complete-but-unverified, finalize→index. `tsc` build clean.
- **Engine = Goose Summon, untouched (no Rust-core change)**. Verified in source (`crates/goose/src/agents/subagent_handler.rs:58-60,158-160,222,259-269`; `summon.rs:1004-1024`): a delegated sub-agent whose recipe sets `response` returns validated structured JSON to the parent. So the agent fans out one `delegate(async, source=tabular-cell-extractor)` per document in waves of `GOOSE_MAX_BACKGROUND_TASKS` (default 5), and the **parent is the single writer** (calls `ingest_results`) — no sub-agent write-contention on the manifest.
- **Phase 2 static artifacts**: `oscar/recipes/tabular-cell-extractor.yaml` (tracked source; packaging copies it into `src/resources/recipes/` and the app copies it to `~/.config/goose/recipes/` at launch) — `response.json_schema` forces the per-document payload; reviewer prompt — verbatim quotes / char offsets / not-found-over-guess / read-the-whole-document — ported from LQ-Grid `.claude/agents/reviewer.md`, dropping its document-grouping metadata as a Phase 5 concern); `ui/desktop/src/components/oscar/recipe/tabularReviewDoctrine.ts` (orchestration block: create → wave-delegate → load → ingest → finalize). Both `oscar-tabular` (dev/prod path resolution + `OSCAR_MATTER_DIR` env, mirroring oscar-fs) and `TABULAR_REVIEW_DOCTRINE` wired into the generic `buildPracticeAreaRecipe.ts` (all 13 areas — batch review is general, not Commercial-only). `tsc --noEmit` clean across ui/desktop.
- **Render decided NATIVE, not MCP App** ([[ADR-113]]): full-window React split (grid + matter-agent rail + citation/document drill, shared state), because the load-bearing requirement is working the grid and the agent *simultaneously*, which the sandboxed-iframe MCP-App host can't do (inline cramped / fullscreen hides chat / no shipped side-by-side mode). LQ-Grid's grid is already React, so native reuses more prior art than re-bundling it into an iframe.
- **Reuse of LQ.AI / LQ-Grid prior art** (per user direction — functionality yes, architecture no): the data model, the two-tier citation/highlight model, the reviewer prompt contract, and (Phase 3) the React grid / SourceView / pdf.js + docx-preview viewers. LQ-Grid's Bun-SSE channel server and localStorage split-persistence explicitly NOT reused — verification/flags live in the one matter-folder manifest, so a review re-opens losslessly and is portable.

**Deferred / carry-forwards**

- Phase 3 (native full-window split surface: route + ported TanStack grid + agent rail + citation panel); Phase 4 (right-pane "Tabular Review" launcher + re-entry IPC + preload); Phase 5 (accretive add_column run, single-cell rerun, "ask the whole table", column-query playbooks via the existing playbooks system, CSV/XLSX/MD export, cost-preview confirm, optional per-column LLM ensemble [[ADR-081]] off by default, and the RAG-baseline fidelity test as the beats-competitors launch gate).
- **Packaging integration (needed before the .deb works)**: bundle `oscar/mcps/tabular/dist` into `ui/desktop/src/resources/mcps/tabular/` via `prepare-oscar-bundle.js` (mirror oscar-fs), and copy `tabular-cell-extractor.yaml` into `~/.config/goose/recipes/` at launch (Summon's verified discovery path; main.ts, mirror `ensureBundledSkillsLink`). In DEV the recipe + MCP load from the wired dist paths.
- **Phase 2 runtime validation (dogfood, deferred to Crostini per the project pattern)**: end-to-end NL → create_review → wave delegate → load → ingest → finalize on a real provider with 5 then 50 documents; confirm the manifest fills wave-by-wave, every `complete` cell has a grounded quote, no silent failures. Pipeline test must use the real configured provider (MiniMax), not a mock (CLAUDE.md). Verify the recipe `response.json_schema` key against the live Recipe schema while there.

**ADRs**: 111 (architecture), 112 (grounding-gate-as-display-precondition), 113 (native render, not MCP App).

**Cross-repo**: only `sarturko-maker/oscar-gc` touched; the new sibling MCP `oscar/mcps/tabular` lives inside it (not a separate repo). No Rust core touch.
...
1. **Recalibrate fix 1's wording for cross-family balance** — Fix 1's MiniMax-vs-Haiku reversal (+35pp / -20pp) is the load-bearing Sprint 32 finding. Per CLAUDE.md ("Reuse over rebuild — Goose": providers are dependency injection, line 56-60), per-family doctrine variants are not an option — doctrine must work across whatever provider the user has wired up. Sprint 33 must recalibrate the wording. Hypothesis: replace the "never path, never prefix, never description" negative-constraint list with a single positive imperative ("pass the slug verbatim as listed"). Separates *when to fire* (positive trigger; same across families) from *how to fire when firing* (slug shape; applies only after the fire decision). Substrate validates cross-family before merge.
...
- **M1 — Skills toggle bug fix ([[ADR-094]])**. Sprint 28's `oscar:skills:toggle` (`main.ts:3144`) treated `current.slugs` as "disabled" regardless of mode; in `mode: 'allow'` the slugs are the ENABLED set, so flipping one Off skill On flipped every other Off skill On at the same time. Fix preserves the existing mode shape on write (`'all'` migrates on first off-toggle; `'allow'` and `'deny'` stay in their shape with correct semantics). Visual harness (`capture-sprint29-m1.{js,sh}`) seeds the missing `mode: 'allow'` starting state Sprint 28's harness skipped and asserts exactly one extra skill flips On after toggling, not all 8. PASS. Screenshots at `docs/screenshots/sprint-29-m1/`.

- **M2 — Drop coming-soon panel stubs ([[ADR-095]])**. `Redlining` (Adeu already lives in Sprint 28's Tools section), `Forum`, `Deadlines` removed from `PanelSectionId` union, `SECTION_META`, `sectionRegistry`, all 13 areas' `defaultPanelSections`, and from Forge's create-area system-prompt enumeration. `PanelSectionStub.tsx` deleted (no consumers). Visual harness asserts the dropped IDs no longer render on Commercial / Commercial Disputes pane and that Tools section still lists "Redlining (Adeu)". PASS.

- **M3 — Skills row presentation matches Tools ([[ADR-096]])**. `humanizeSlug` helper at `skills/humanizeSlug.ts` title-cases the slug on hyphen boundaries and expands a closed 18-entry legal-domain acronym map (NDA / MSA / SaaS / IP / AI / EU / UK / US / GDPR / M&A / IPO / KYC / AML / RFP / SOW / LOI / POC / T&Cs). `joinSkills` (`skillStore.ts:135`) writes `humanizeSlug(slug)` into `SkillEntry.name`. `.oscar__skills-name` CSS switches from `mono-editorial` → `sans-editorial` 12/500 to mirror Tools. Slug stays addressable on `data-testid`, `data-source`, and the row's `title` attribute. Visual harness asserts `nda-review` → "NDA Review", `saas-msa-review` → "SaaS MSA Review", `amendment-history` → "Amendment History"; computed font-family is `Outfit, ...sans-serif` not mono. PASS.

- **M4 — Skills section surface zone + collapsed directory ([[ADR-097]])**. SkillsSection splits into two zones: "In this matter" (enabled only, surface, always visible) and "All skills" (everything, behind a `<details>`-style disclosure). Surface zone shows compact rows with the Off chip; directory carries the full row plus drop affordance + bundled tag + description + delete (for user-added). Auto-opens the directory when surface is empty (lawyer-with-nothing-on hand-off). React-local collapse state. Visual harness exercises three states: default all-On + directory collapsed; one Off → moved to directory; all Off → surface empty + directory auto-open. PASS.

- **M5 — Edit splits into in-pane manual form + Forge entry ([[ADR-098]])**. The header `Edit` link (Sprint 20-M7 / ADR-088) is no longer a Forge deep-link. It now toggles an in-pane editing mode for the MatterFacts section: form fields for `name`, `subject_label`, `counterparty.role`/`name`, `kind`, `stakeholder`, kind-conditional extras, `privileged`, `key_facts` — reusing `PracticeAreaShape` so the editor mirrors `NewMatterDialog`'s per-area shape. New `MatterFactsEditor.tsx`; new `oscar:matters:update` IPC (Zod-validated `UpdateMatterInputSchema`); registry write + matter.md re-render via existing `renderMatterMd`. Slug is immutable. The footer carries a labelled "Ask Forge to change this area's loadout" button with a one-sentence pitch — preserves the Forge route for power changes (description, panel sections, skill loadout, MCP loadout) and gives lawyers the orientation Sprint 28's direct deep-link skipped. `RightPaneContext` extended with `editingFacts`/`begin`/`end` so the header (Edit/Cancel) and the section (form/display) stay coordinated. `usePanelReader` gains `refresh` so the display catches up immediately after save. Visual harness verifies display → form → save (registry + matter.md updated; `privileged` flipped; name rewritten; key-facts body appended) → form-close → re-open → Forge deep-link navigates through to a fresh Forge session. PASS.

- **M6 — On-demand playbook discovery block ([[ADR-099]])**. The Sprint 20-M4 / ADR-085 architecture had three layers: Layer 1 (always-on injection — wired), Layer 2 (on-demand read via oscar-fs / computercontroller — wired but invisible: the system prompt never enumerated the available playbooks), Layer 3 (semantic retrieval — deferred). The visibility gap meant lawyers uploading a playbook saw nothing happen unless they paste the filename into a prompt. Patch: new `renderOnDemandPlaybooksBlock` helper + `oscar:playbooks:render-on-demand-block` IPC + renderer wrapper composed between Layer 1's block and the skills block in `buildPracticeAreaRecipe`. The block enumerates every non-always-on playbook with relative path + scope + size; for text formats a first-line peek (≤ 80 chars) provides a purpose hint. Probe harness asserts: empty case → null block (slot skipped); two text playbooks → both listed with hints; flipping one always-on → drops from on-demand block (now belongs to Layer 1). PASS. Inspection memo at `docs/sprint-29/playbook-wiring-inspection.md` documents the three layers + the gap + what carries to Crostini dogfood (Layer 3 / "10-playbook pick-the-right-one" remains the right deferred work).

- **M7 — Multi-chat per matter research-only ([[ADR-100]])**. Per Sprint 29 brief's Issue 7 scope: research only, implementation Sprint 30+. Verified directly against Goose source (per CLAUDE.md "code prevails over documentation" — the `goose-docs.ai` URLs probed for session/compaction guidance returned 404, so source was authoritative): (a) Goose auto-compacts each session at 80% of context-limit (`context_mgmt/mod.rs:185` + `agents/agent.rs:1496`), server-side, transparent to renderer via `AgentEvent::HistoryReplaced`; (b) sessions are independent (`session/session_manager.rs:303`); (c) `/compact` slash command exists at `goose-cli/src/session/input.rs:205`; (d) `chatrecall` platform extension (`agents/platform_extensions/chatrecall.rs`, on by default per Sprint 18 ADR-063) gives the agent on-demand keyword + load-by-id search across past sessions. Conclusion: the matter is a container, not a thread — multi-chat needs no summary-on-spawn wiring. Sprint 30 implementation follows Sprint 27's per-partner pattern ([[ADR-092]] sprint27-multi-session-per-partner) verbatim: schema `MatterEntry.session_id` → `sessions: SessionRef[]` with lazy v2→v3 read-time migration; inline session list on matter row (cap 5 + overflow); IPC PREPEND-on-bind with dedupe-by-id; reserved unbind for future delete UI. ~5 files touched, no Rust core touch. Memo at `docs/sprint-29/multi-chat-research-memo.md` with full source pointers + affected-surface table.

- **M8 — .deb + close-out**. `bash scripts/build-oscar-deb.sh` → `out/make/deb/x64/oscar-gc_1.34.0_amd64.deb` (249 MB). `tsc --noEmit` clean throughout. Two-line Sprint 28 carry-forward cleanup: `main.ts:3073` and `preload.ts:388` ADR refs disambiguated for the duplicate ADR numbering (091 / 092 each occur twice in the repo; comments now name the slug suffix to clarify which sibling is meant — the Sprint 28 carry-forward note framed these as "non-existent ADRs" but on inspection both targets exist and the underlying issue was the duplicate-numbering, not staleness).

**ADRs**: 094, 095, 096, 097, 098, 099, 100.

**Deferred**: nothing scoped within the seven issues. Threads that surfaced but stayed out per the brief: Layer 3 semantic playbook retrieval (the "10-playbook pick-the-right-one" story), Sprint 30 multi-chat implementation, Forge Mode B/C/D system-prompt refresh (still references the dropped `Redlining`/`Forum`/`Deadlines` panel ids only in its enumeration list — fixed in M2 in the same edit).

**Carry-forwards**:

- **Arturs's next-tranche Crostini feedback** ("I will have more comments but let's deal with these first") — Sprint 30 brief.
- **Editable session labels** + **session-delete UI** — Sprint 27 reserved IPC + Sprint 30 multi-chat both consume the same affordance. Bundle into Sprint 30.
- **Duplicate ADR numbering** — the repo has two each of ADR-090, ADR-091, ADR-092. M8's disambiguating comment refactor mitigates per-file; a one-shot rename pass would clean it up properly. Cosmetic; defer until a quiet sprint.
- **`pnpm package` sub-recipes copy** (Sprint 28 carry-forward) — already in place this session; would benefit from being baked into the `package` script's `prepackage` hook.

**Crostini dogfood**: Arturs hand-test against six exit criteria (1) toggling one Off skill On flips ONLY that skill On (not all); (2) Commercial pane has no Redlining/Forum/Deadlines "coming soon" sections; Tools still lists Redlining (Adeu); (3) Skill rows read as "NDA Review" / "SaaS MSA Review", not kebab-slugs; visual weight matches Tools; (4) Skills section shows "In this matter" surface + "All skills" collapsed directory; flipping one off moves it to the directory; (5) clicking Edit opens an in-pane form (not Forge); Save persists; the "Ask Forge to change this area's loadout" button still routes to Forge; (6) uploading a playbook (not always-on) and asking the agent "what playbooks are available" surfaces the on-demand block.
