...
- **Live MiniMax agent-orchestration dogfood** (the gating fan-out question, [[ADR-107]] delegate-shyness risk): **set up + ready** (binary built with the recipe discoverable at Summon's path + the tabular MCP bundled/spawn-smoke-green; markdown fixtures + 50-doc generator; a real Commercial matter exists) but **not run this session** — the engine *mechanics* are proven deterministically (demo-manifest), the live agent driving create_review→delegate-per-doc→load→ingest on MiniMax is the explicit remaining Stage A step. Run via `dogfood.sh` (boot → open the matter for a session → pair-send the NL ask → read the manifest). Write the engine-verdict ADR (no per-model doctrine branch; malformed-payload path is defense-in-depth behind core's `final_output` schema gate) with the result in hand.
- **`BaseChat` live streaming in the bare route**: the rail shell + `ChatProvider` render (the screenshot showed the no-session CTA because the matter's `session_id` was null); BaseChat mounts only with a bound session — live rail streaming is pending the session-bound run above (the Plan-agent code-read argues it works: `useNavigationContextSafe` + no `activeSessions` dep).
